...
const getListings = async (filters, isAdmin, page = 1, limit = 10) => {
  ...
  if (filters.property_type) {
    ...
    countQuery += ` AND property_type ILIKE $${count}`;
  ...
  if (filters.suburb) {
    ...
    countQuery += ` AND suburb ILIKE $${count}`;
  ...
  if (filters.keyword) {
    ...
    countQuery += ` AND (title ILIKE $${count} OR description ILIKE $${count} OR address ILIKE $${count})`;
